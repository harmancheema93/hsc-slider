...
function hsc_slider_shortcode( $atts){
	$atts = shortcode_atts(array( 'id' => '', 'name' => '', 'dots' => 'true', 'arrows' => 'true', 'infinte' => 'true', 'class' => 'hsc-slider' ), $atts);
	$sliderId = $atts['id'];
	...
	foreach($slides as $slide){
		...
		(isset($slide['style']) && $slide['style'] == 'on') ? $style = 'style="background: url('.$image.'); background-repeat: no-repeat; background-size: cover; background-position: center;"' : $style = '';
		switch($slide['heading_style']){
			case 'h1':
				$headingStyle = '<h1 class="h1">'.$heading.'</h1>';
			break;
			case 'h2':
				$headingStyle = '<h2 class="h1">'.$heading.'</h2>';
			break;
			case 'h3':
				$headingStyle = '<h3 class="h1">'.$heading.'</h3>';
			break;
			case 'h4':
				$headingStyle = '<h4 class="h1">'.$heading.'</h3>';
			break;
			case 'p':
				$headingStyle = '<p class="h1">'.$heading.'</p>';
			break;
			default:
				$headingStyle = '<h2 class="h1">'.$heading.'</h2>';
			break;
		...
		if($visibility != 'off'){
			...
			if($button_text){
				$html .= '<a class="button btn btn-primary" href="'.$button_link.'">'.$button_text.'</a>';
			}
			$html .='</div>';
			$html .= '</div></div>';
